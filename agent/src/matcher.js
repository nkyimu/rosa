...
import { publicClient, walletClient, getIntentRegistry, getCircleFactory, IntentType, intentRegistryAbi, circleFactoryAbi, saveCircleAbi, } from "./contracts.js";
import { CONTRACT_ADDRESSES, agentAccount, MATCHER_CONFIG, DRY_RUN, } from "./config.js";
// ─── Intent Params Schema ─────────────────────────────────────────────────────
// Encoded as: (uint256 contributionAmount, uint256 cycleDuration, uint8 preferredSize)
const INTENT_PARAMS_SCHEMA = parseAbiParameters("uint256 contributionAmount, uint256 cycleDuration, uint8 preferredSize");
// ─── IntentMatcher ────────────────────────────────────────────────────────────
export class IntentMatcher {
    constructor() {
        this.intentRegistry = getIntentRegistry();
        this.circleFactory = getCircleFactory();
    }
    /** Decode JOIN_CIRCLE intent params from bytes */
    decodeParams(paramsHash) {
    ...
    async scanJoinIntents() {
        ...
        for (const raw of rawIntents) {
            if (raw.fulfilled || raw.cancelled)
                continue;
            if (raw.expiresAt > 0n && raw.expiresAt < now)
                continue;
        ...
        for (const anchor of sorted) {
            ...
            for (const candidate of sorted) {
                if (used.has(candidate.id))
                    continue;
                if (candidate.params.cycleDuration !== anchor.params.cycleDuration)
                    continue;
                // Check ±10% band
                ...
                if (group.length >= MATCHER_CONFIG.maxGroupSize)
                    break;
            }
     ...
     */
    async matchAndDeploy(group) {
        if (!walletClient || !agentAccount) {
            throw new Error("Wallet client not configured — set AGENT_PRIVATE_KEY");
        }
        console.log(`[matcher] Deploying circle for ${group.intents.length} members ` +
        ...
        if (DRY_RUN) {
            ...
            console.log(`[matcher] 🏜️  DRY_RUN: Would batch-fulfill ${group.intents.length} intents`);
            return demoAddress;
        }
        ...
        const circleCreatedLog = createReceipt.logs.find((log) => log.topics[0] === "0x" + "CircleCreated".padEnd(64, "0") // rough check
        );
        ...
        if (!circleAddress)
            throw new Error("No circle address returned");
        ...
        const solution = encodeAbiParameters(parseAbiParameters("address circleAddress"), [circleAddress]);
