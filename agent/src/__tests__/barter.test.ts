...
describe("Barter Matching Module", () => {
  ...
  describe("submitBarterIntent", () => {
    it("should submit a barter intent (mock: assume ELDER tier)", async () => {
      ...
      const intentId = await submitBarterIntent(
        "0x1111111111111111111111111111111111111111",
        "web design (40 hours)",
        "childcare (10 hrs/week)"
      );
  ...
  describe("matchBarterIntents", () => {
    it("should find matching intents based on keyword overlap", async () => {
      ...
      const intentA = await submitBarterIntent(
        "0x1111111111111111111111111111111111111111",
        "web design (40 hours)",
        "childcare (10 hrs/week)"
      );

      const intentB = await submitBarterIntent(
    ...
    it("should sort matches by compatibility score", async () => {
      ...
      const intentC = await submitBarterIntent(
        "0x3333333333333333333333333333333333333333",
        "services",
        "help"
      );

      const intentD = await submitBarterIntent(
    ...
    it("should not match already matched intents", async () => {
      ...
      const hasSettledIntent = matches.some(
        (m) =>
          (m.intentA.id === intentA && m.intentB.id === intentB) ||
  ...
  describe("getAllBarterIntents", () => {
    it("should return all intents regardless of status", async () => {
      ...
      const intent1 = await submitBarterIntent(
        "0x2222222222222222222222222222222222222222",
        "childcare",
        "web design"
      );

      const intent2 = await submitBarterIntent(
        "0x3333333333333333333333333333333333333333",
        "graphic design",
        "marketing"
      );
