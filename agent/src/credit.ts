...
import { publicClient, walletClient, agentAccount } from "./contracts.js";
import { CONTRACT_ADDRESSES, DRY_RUN } from "./config.js";
...
export enum CreditStatus {
  ...
  DEFAULT = "DEFAULT",     // Defaulted after grace period
}
...
export interface CreditLine {
  ...
  status: CreditStatus;
  circleIdRequired: number; // Borrower must join this circle to activate
 ...
 */
export async function issueCreditLine(
  issuerAddress: string,
  borrowerAddress: string,
  amount: bigint,
  termWeeks: number,
  ...
  const assessment = await assessCreditworthiness(
    issuerAddress,
    borrowerAddress,
    amount * BigInt(5) // Mock: assume issuer has 5x the credit amount in portfolio
  );
  ...
  if (amount > assessment.maxAmount) {
    throw new Error(
      `Amount ${amount} exceeds max ${assessment.maxAmount} (${assessment.reason})`
    );
  }
  ...
  addActivity(
    "CIRCLE_HEALTH",
    `Credit issued: ${issuerAddress.slice(0, 6)}... → ${borrowerAddress.slice(0, 6)}...`,
    `${amount / BigInt("1000000000000000000")} cUSD, ${termWeeks} weeks, required circle ${circleIdRequired}`,
    0.9
  );
...
export async function activateCredit(
  ...
  addActivity(
    "EXECUTE",
    `Credit activated: ${creditId}`,
    `Borrower ${borrowerAddress.slice(0, 6)}... joined circle and received ${credit.amount / BigInt("1000000000000000000")} cUSD`,
    0.9
  );
}
...
export async function repayCredit(
  ...
  addActivity(
    "EXECUTE",
    `Credit repaid: ${creditId}`,
    `Borrower ${repayerAddress.slice(0, 6)}... repaid ${credit.amount / BigInt("1000000000000000000")} cUSD`,
    0.95
  );
}
 ...
 */
export function getCreditLines(address: string): {
  issued: CreditLine[];
  received: CreditLine[];
} {
  const issued: CreditLine[] = [];
  const received: CreditLine[] = [];
 ...
 */
export function getCredit(creditId: string): CreditLine | undefined {
 ...
 */
export async function checkCreditHealth(): Promise<string[]> {
  ...
  for (const [creditId, credit] of creditLines) {
    ...
    if (now > gracePeriodEnd) {
      ...
      addActivity(
        "CIRCLE_HEALTH",
        `Credit defaulted: ${creditId}`,
        `Issuer ${credit.issuerAddress.slice(0, 6)}..., borrower ${credit.borrowerAddress.slice(0, 6)}...`,
        0.95
      );
    }
  }
 ...
 * @param creditId Credit line ID
 */
export async function handleDefault(creditId: string): Promise<void> {
  const credit = creditLines.get(creditId);
  if (!credit) {
    throw new Error(`Credit ${creditId} not found`);
  }
  ...
  addActivity(
    "CIRCLE_HEALTH",
    `Default handled: ${creditId}`,
    `Reputation penalty triggered for issuer and borrower`,
    0.9
  );
}
 ...
 */
export function getAllCredits(): CreditLine[] {
