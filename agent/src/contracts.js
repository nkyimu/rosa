...
var crypto3 = nc2 && typeof nc2 === "object" && "webcrypto" in nc2 ? nc2.webcrypto : nc2 && typeof nc2 === "object" && ("randomBytes" in nc2) ? nc2 : undefined;
/*! noble-hashes - MIT License (c) 2022 Paul Miller (paulmillr.com) */
function isBytes3(a) {
  return a instanceof Uint8Array || ArrayBuffer.isView(a) && a.constructor.name === "Uint8Array";
}
...
function isBytes22(a) {
  return a instanceof Uint8Array || ArrayBuffer.isView(a) && a.constructor.name === "Uint8Array";
}
function abytes22(item) {
  if (!isBytes22(item))
    throw new Error("Uint8Array expected");
}
...
var validatorFns2 = {
  ...
  boolean: (val) => typeof val === "boolean",
  string: (val) => typeof val === "string",
  stringOrUint8Array: (val) => typeof val === "string" || isBytes22(val),
  isSafeInteger: (val) => Number.isSafeInteger(val),
  array: (val) => Array.isArray(val),
  field: (val, object) => object.Fp.isValid(val),
  hash: (val) => typeof val === "function" && Number.isSafeInteger(val.outputLen)
...
function sqrt3mod42(Fp, n) {
  const p1div4 = (Fp.ORDER + _1n22) / _4n3;
  const root = Fp.pow(n, p1div4);
  if (!Fp.eql(Fp.sqr(root), n))
    throw new Error("Cannot find square root");
  return root;
...
function tonelliShanks2(P) {
  ...
  return function tonelliSlow(Fp, n) {
    ...
    while (!Fp.eql(t, Fp.ONE)) {
      ...
      const exponent = _1n22 << BigInt(M - i - 1);
      const b = Fp.pow(c, exponent);
      M = i;
      c = Fp.sqr(b);
      t = Fp.mul(t, c);
...
function FpLegendre2(Fp, n) {
  const p1mod2 = (Fp.ORDER - _1n22) / _2n5;
  const powered = Fp.pow(n, p1mod2);
  const yes = Fp.eql(powered, Fp.ONE);
  const zero = Fp.eql(powered, Fp.ZERO);
  const no = Fp.eql(powered, Fp.neg(Fp.ONE));
...
function Field2(ORDER, bitLen22, isLE22 = false, redef = {}) {
  if (ORDER <= _0n22)
    throw new Error("invalid field: expected ORDER > 0, got " + ORDER);
  const { nBitLength: BITS, nByteLength: BYTES } = nLength2(ORDER, bitLen22);
  if (BYTES > 2048)
    throw new Error("invalid field: expected ORDER of <= 2048 bytes");
  let sqrtP;
  const f = Object.freeze({
    ...
    toBytes: (num2) => isLE22 ? numberToBytesLE2(num2, BYTES) : numberToBytesBE2(num2, BYTES),
    fromBytes: (bytes) => {
      if (bytes.length !== BYTES)
        throw new Error("Field.fromBytes: expected " + BYTES + " bytes, got " + bytes.length);
      return isLE22 ? bytesToNumberLE2(bytes) : bytesToNumberBE2(bytes);
    },
...
function mapHashToField2(key, fieldOrder, isLE22 = false) {
  ...
  if (len < 16 || len < minLen || len > 1024)
    throw new Error("expected " + minLen + "-1024 bytes of input, got " + len);
  const num2 = isLE22 ? bytesToNumberLE2(key) : bytesToNumberBE2(key);
  const reduced = mod2(num2, fieldOrder - _1n22) + _1n22;
  return isLE22 ? numberToBytesLE2(reduced, fieldLen) : numberToBytesBE2(reduced, fieldLen);
}
...
function weierstrassPoints2(opts) {
  ...
  const toBytes22 = CURVE.toBytes || ((_c, point, _isCompressed) => {
    const a = point.toAffine();
    return concatBytes22(Uint8Array.from([4]), Fp.toBytes(a.x), Fp.toBytes(a.y));
  });
  ...
  if (!isValidXY(CURVE.Gx, CURVE.Gy))
    throw new Error("bad curve params: generator point");
  const _4a3 = Fp.mul(Fp.pow(CURVE.a, _3n22), _4n22);
  const _27b2 = Fp.mul(Fp.sqr(CURVE.b), BigInt(27));
  if (Fp.is0(Fp.add(_4a3, _27b2)))
    throw new Error("bad curve params: a or b");
  function isWithinCurveOrder(num2) {
    return inRange2(num2, _1n42, CURVE.n);
  }
  function normPrivateKeyToScalar(key) {
    const { allowedPrivateKeyLengths: lengths, nByteLength, wrapPrivateKey, n: N } = CURVE;
    if (lengths && typeof key !== "bigint") {
    ...
    if (wrapPrivateKey)
      num2 = mod2(num2, N);
    aInRange2("private key", num2, _1n42, N);
    return num2;
  }
  function aprjpoint(other) {
  ...
  class Point2 {
    ...
    multiplyUnsafe(sc) {
      const { endo: endo2, n: N } = CURVE;
      aInRange2("scalar", sc, _0n42, N);
      const I = Point2.ZERO;
      if (sc === _0n42)
        return I;
      if (this.is0() || sc === _1n42)
        return this;
      if (!endo2 || wnaf.hasPrecomputes(this))
    ...
    multiply(scalar) {
      const { endo: endo2, n: N } = CURVE;
      aInRange2("scalar", scalar, _1n42, N);
      let point, fake;
      if (endo2) {
        const { k1neg, k1, k2neg, k2 } = endo2.splitScalar(scalar);
        let { p: k1p, f: f1p } = this.wNAF(k1);
...
function weierstrass2(curveDef) {
  ...
  class Signature {
    constructor(r, s, recovery) {
      aInRange2("r", r, _1n42, CURVE_ORDER);
      aInRange2("s", s, _1n42, CURVE_ORDER);
      this.r = r;
      this.s = s;
      if (recovery != null)
        this.recovery = recovery;
  ...
  function int2octets(num2) {
    aInRange2("num < 2^" + nBitLength, num2, _0n42, ORDER_MASK);
    return numberToBytesBE2(num2, nByteLength);
  }
  function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
    if (["recovered", "canonical"].some((k) => (k in opts)))
      throw new Error("sign() legacy options not supported");
    const { hash: hash3, randomBytes: randomBytes22 } = CURVE;
    let { lowS, prehash, extraEntropy: ent } = opts;
    if (lowS == null)
      lowS = true;
    ...
    const seedArgs = [int2octets(d), int2octets(h1int)];
    if (ent != null && ent !== false) {
      const e = ent === true ? randomBytes22(Fp.BYTES) : ent;
      seedArgs.push(ensureBytes2("extraEntropy", e));
    }
    const seed = concatBytes22(...seedArgs);
    const m = h1int;
    function k2sig(kBytes) {
      const k = bits2int(kBytes);
      if (!isWithinCurveOrder(k))
  ...
  function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    if (format !== undefined && format !== "compact" && format !== "der")
      throw new Error("format must be compact or der");
    const isHex2 = typeof sg === "string" || isBytes22(sg);
    const isObj = !isHex2 && !format && typeof sg === "object" && sg !== null && typeof sg.r === "bigint" && typeof sg.s === "bigint";
    if (!isHex2 && !isObj)
      throw new Error("invalid signature, expected Uint8Array, hex string or Signature instance");
    let _sig = undefined;
...
var _1n52 = BigInt(1);
var _2n32 = BigInt(2);
var divNearest2 = (a, b) => (a + b / _2n32) / b;
function sqrtMod2(y) {
  const P = secp256k1P2;
  ...
  const b2 = y * y * y % P;
  const b3 = b2 * b2 * y % P;
  const b6 = pow22(b3, _3n32, P) * b3 % P;
  const b9 = pow22(b6, _3n32, P) * b3 % P;
  const b11 = pow22(b9, _2n32, P) * b2 % P;
  const b22 = pow22(b11, _11n, P) * b11 % P;
  const b44 = pow22(b22, _22n, P) * b22 % P;
  const b88 = pow22(b44, _44n, P) * b44 % P;
  const b176 = pow22(b88, _88n, P) * b88 % P;
  const b220 = pow22(b176, _44n, P) * b44 % P;
  const b223 = pow22(b220, _3n32, P) * b3 % P;
  const t1 = pow22(b223, _23n, P) * b22 % P;
  const t2 = pow22(t1, _6n, P) * b2 % P;
  const root = pow22(t2, _2n32, P);
  if (!Fpk12.eql(Fpk12.sqr(root), y))
    throw new Error("Cannot find square root");
  return root;
}
var Fpk12 = Field2(secp256k1P2, undefined, undefined, { sqrt: sqrtMod2 });
var secp256k12 = createCurve2({
  ...
  endo: {
    ...
    splitScalar: (k) => {
      const n = secp256k1N2;
      const a1 = BigInt("0x3086d221a7d46bcde86c90e49284eb15");
      const b1 = -_1n52 * BigInt("0xe4437ed6010e88286f547fa90abfe4c3");
      const a2 = BigInt("0x114ca50f7a8e2f3f657c1108d9d44cfd8");
      const b2 = a1;
      const POW_2_128 = BigInt("0x100000000000000000000000000000000");
...
class BaseError4 extends Error {
  constructor(shortMessage, args = {}) {
    ...
    const docsUrl = errorConfig2.getDocsUrl?.({ ...args, docsPath: docsPath8 });
    const message = [
      shortMessage || "An error occurred.",
      "",
      ...args.metaMessages ? [...args.metaMessages, ""] : [],
...
class SizeOverflowError4 extends BaseError4 {
  constructor({ givenSize, maxSize }) {
    super(`Size cannot exceed ${maxSize} bytes. Given size: ${givenSize} bytes.`, { name: "SizeOverflowError" });
  }
}
...
class SliceOffsetOutOfBoundsError4 extends BaseError4 {
...
class SizeExceedsPaddingSizeError4 extends BaseError4 {
...
function pad4(hexOrBytes, { dir, size: size22 = 32 } = {}) {
  if (typeof hexOrBytes === "string")
...
function padBytes2(bytes, { dir, size: size22 = 32 } = {}) {
  if (size22 === null)
    return bytes;
...
var hexes22 = /* @__PURE__ */ Array.from({ length: 256 }, (_v, i) => i.toString(16).padStart(2, "0"));
...
function boolToHex2(value, opts = {}) {
  const hex = `0x${Number(value)}`;
  if (typeof opts.size === "number") {
    assertSize4(hex, { size: opts.size });
    return pad4(hex, { size: opts.size });
  }
  return hex;
}
function bytesToHex22(value, opts = {}) {
  ...
  if (typeof opts.size === "number") {
    assertSize4(hex, { size: opts.size });
    return pad4(hex, { dir: "right", size: opts.size });
  }
  return hex;
}
...
function boolToBytes2(value, opts = {}) {
  ...
  if (typeof opts.size === "number") {
    ...
    return pad4(bytes, { size: opts.size });
  }
  return bytes;
}
var charCodeMap3 = {
...
function hexToBytes22(hex_, opts = {}) {
  ...
  if (opts.size) {
    assertSize4(hex, { size: opts.size });
    hex = pad4(hex, { dir: "right", size: opts.size });
  }
  let hexString = hex.slice(2);
  if (hexString.length % 2)
    hexString = `0${hexString}`;
...
function stringToBytes2(value, opts = {}) {
  const bytes = encoder22.encode(value);
  if (typeof opts.size === "number") {
    assertSize4(bytes, { size: opts.size });
    return pad4(bytes, { dir: "right", size: opts.size });
  }
  return bytes;
}
function assertSize4(hexOrBytes, { size: size22 }) {
  if (size5(hexOrBytes) > size22)
    throw new SizeOverflowError4({
      givenSize: size5(hexOrBytes),
      maxSize: size22
    });
}
...
function hexToNumber22(hex, opts = {}) {
  const value = hexToBigInt2(hex, opts);
  const number = Number(value);
  if (!Number.isSafeInteger(number))
    throw new IntegerOutOfRangeError3({
      max: `${Number.MAX_SAFE_INTEGER}`,
...
var SHA3_ROTL2 = [];
var _SHA3_IOTA2 = [];
for (let round = 0, R = _1n62, x = 1, y = 0;round < 24; round++) {
  [x, y] = [y, (2 * x + 3 * y) % 5];
  SHA3_PI2.push(2 * (5 * y + x));
  SHA3_ROTL2.push((round + 1) * (round + 2) / 2 % 64);
  let t = _0n62;
  for (let j = 0;j < 7; j++) {
    R = (R << _1n62 ^ (R >> _7n2) * _0x71n2) % _256n2;
    if (R & _2n42)
...
function keccak2563(value, to_) {
  const to = to_ || "hex";
  const bytes = keccak_2562(isHex2(value, { strict: false }) ? toBytes22(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex3(bytes);
}
...
function checksumAddress2(address_, chainId) {
  ...
  const hexAddress = chainId ? `${chainId}${address_.toLowerCase()}` : address_.substring(2).toLowerCase();
  const hash3 = keccak2563(stringToBytes2(hexAddress), "bytes");
  const address = (chainId ? hexAddress.substring(`${chainId}0x`.length) : hexAddress).split("");
  for (let i = 0;i < 40; i += 2) {
...
function toAccount(source) {
  if (typeof source === "string") {
    if (!isAddress2(source, { strict: false }))
      throw new InvalidAddressError3({ address: source });
    return {
      address: source,
      type: "json-rpc"
    };
  ...
  if (!isAddress2(source.address, { strict: false }))
    throw new InvalidAddressError3({ address: source.address });
  return {
...
async function sign({ hash: hash3, privateKey, to = "object" }) {
  const { r, s, recovery } = secp256k12.sign(hash3.slice(2), privateKey.slice(2), {
    lowS: true,
    extraEntropy: isHex2(extraEntropy, { strict: false }) ? hexToBytes22(extraEntropy) : extraEntropy
  });
  const signature = {
...
function concat3(values) {
  ...
  return concatBytes32(values);
}
function concatBytes32(values) {
  let length = 0;
  for (const arr of values) {
    length += arr.length;
  }
...
class NegativeOffsetError3 extends BaseError4 {
  constructor({ offset }) {
    super(`Offset \`${offset}\` cannot be negative.`, {
      name: "NegativeOffsetError"
    });
...
class PositionOutOfBoundsError3 extends BaseError4 {
  constructor({ length, position }) {
    super(`Position \`${position}\` is out of bounds (\`0 < position < ${length}\`).`, { name: "PositionOutOfBoundsError" });
  }
}

class RecursiveReadLimitExceededError3 extends BaseError4 {
  constructor({ count, limit }) {
    super(`Recursive read limit of \`${limit}\` exceeded (recursive read count: \`${count}\`).`, { name: "RecursiveReadLimitExceededError" });
  }
}
...
function createCursor2(bytes, { recursiveReadLimit = 8192 } = {}) {
...
function toRlp2(bytes, to = "hex") {
  const encodable = getEncodable3(bytes);
  const cursor = createCursor2(new Uint8Array(encodable.length));
  encodable.encode(cursor);
  if (to === "hex")
    return bytesToHex22(cursor.bytes);
  return cursor.bytes;
}
function getEncodable3(bytes) {
  if (Array.isArray(bytes))
    return getEncodableList3(bytes.map((x) => getEncodable3(x)));
  return getEncodableBytes3(bytes);
}
...
function getEncodableBytes3(bytesOrHex) {
  const bytes = typeof bytesOrHex === "string" ? hexToBytes22(bytesOrHex) : bytesOrHex;
  const sizeOfBytesLength = getSizeOfLength3(bytes.length);
  const length = (() => {
    if (bytes.length === 1 && bytes[0] < 128)
      return 1;
    if (bytes.length <= 55)
...
function hashAuthorization2(parameters) {
  const { chainId, nonce, to } = parameters;
  const address = parameters.contractAddress ?? parameters.address;
  const hash3 = keccak2563(concatHex2([
    "0x05",
    toRlp2([
...
function toPrefixedMessage2(message_) {
  const message = (() => {
    ...
    if (typeof message_.raw === "string")
      return message_.raw;
    return bytesToHex22(message_.raw);
  })();
  const prefix = stringToHex2(`${presignMessagePrefix2}${size5(message)}`);
  return concat3([prefix, message]);
}
function hashMessage2(message, to_) {
  return keccak2563(toPrefixedMessage2(message), to_);
}
...
function blobsToCommitments2(parameters) {
  const { kzg } = parameters;
  const to = parameters.to ?? (typeof parameters.blobs[0] === "string" ? "hex" : "bytes");
  const blobs = typeof parameters.blobs[0] === "string" ? parameters.blobs.map((x) => hexToBytes22(x)) : parameters.blobs;
  const commitments = [];
  for (const blob of blobs)
    commitments.push(Uint8Array.from(kzg.blobToKzgCommitment(blob)));
  return to === "bytes" ? commitments : commitments.map((x) => bytesToHex22(x));
}
function blobsToProofs2(parameters) {
  const { kzg } = parameters;
  const to = parameters.to ?? (typeof parameters.blobs[0] === "string" ? "hex" : "bytes");
  const blobs = typeof parameters.blobs[0] === "string" ? parameters.blobs.map((x) => hexToBytes22(x)) : parameters.blobs;
  const commitments = typeof parameters.commitments[0] === "string" ? parameters.commitments.map((x) => hexToBytes22(x)) : parameters.commitments;
  const proofs = [];
  for (let i = 0;i < blobs.length; i++) {
    const blob = blobs[i];
    const commitment = commitments[i];
  ...
  return to === "bytes" ? proofs : proofs.map((x) => bytesToHex22(x));
}
var sha25622 = sha2564;
function sha25632(value, to_) {
  const to = to_ || "hex";
  const bytes = sha25622(isHex2(value, { strict: false }) ? toBytes22(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex3(bytes);
}
function commitmentToVersionedHash2(parameters) {
  ...
  return to === "bytes" ? versionedHash : bytesToHex22(versionedHash);
}
...
class BlobSizeTooLargeError2 extends BaseError4 {
...
class EmptyBlobError2 extends BaseError4 {
  constructor() {
    super("Blob data must not be empty.", { name: "EmptyBlobError" });
  }
}

class InvalidVersionedHashSizeError2 extends BaseError4 {
  constructor({ hash: hash3, size: size22 }) {
...
class InvalidVersionedHashVersionError2 extends BaseError4 {
  constructor({ hash: hash3, version: version22 }) {
...
function toBlobs2(parameters) {
  const to = parameters.to ?? (typeof parameters.data === "string" ? "hex" : "bytes");
  const data = typeof parameters.data === "string" ? hexToBytes22(parameters.data) : parameters.data;
  const size_ = size5(data);
  if (!size_)
  ...
  return to === "bytes" ? blobs.map((x) => x.bytes) : blobs.map((x) => bytesToHex22(x.bytes));
}
...
function slice4(value, start, end, { strict } = {}) {
  if (isHex2(value, { strict: false }))
...
function assertStartOffset4(value, start) {
  if (typeof start === "number" && start > 0 && start > size5(value) - 1)
...
function assertEndOffset4(value, start, end) {
  if (typeof start === "number" && typeof end === "number" && size5(value) !== end - start) {
    throw new SliceOffsetOutOfBoundsError4({
      offset: end,
      position: "end",
      size: size5(value)
...
function sliceBytes2(value_, start, end, { strict } = {}) {
  assertStartOffset4(value_, start);
  const value = value_.slice(start, end);
  if (strict)
    assertEndOffset4(value, start, end);
  return value;
}
...
function assertTransactionEIP48442(transaction) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash3 of blobVersionedHashes) {
      const size_ = size5(hash3);
      const version22 = hexToNumber22(slice4(hash3, 0, 1));
      if (size_ !== 32)
        throw new InvalidVersionedHashSizeError2({ hash: hash3, size: size_ });
      if (version22 !== versionedHashVersionKzg2)
...
function serializeTransactionEIP48442(transaction, signature) {
  ...
  let sidecars = transaction.sidecars;
  if (transaction.blobs && (typeof blobVersionedHashes === "undefined" || typeof sidecars === "undefined")) {
    const blobs2 = typeof transaction.blobs[0] === "string" ? transaction.blobs : transaction.blobs.map((x) => bytesToHex22(x));
    const kzg = transaction.kzg;
...
async function signTransaction2(parameters) {
  const { privateKey, transaction, serializer = serializeTransaction4 } = parameters;
  ...
  const signature = await sign({
    hash: keccak2563(await serializer(signableTransaction)),
    privateKey
  });
  return await serializer(transaction, signature);
}

class AbiEncodingArrayLengthMismatchError2 extends BaseError4 {
  constructor({ expectedLength, givenLength, type }) {
    super([
      `ABI encoding array length mismatch for type ${type}.`,
      `Expected length: ${expectedLength}`,
...
class AbiEncodingBytesSizeMismatchError2 extends BaseError4 {
  constructor({ expectedSize, value }) {
    super(`Size of bytes "${value}" (bytes${size5(value)}) does not match expected size (bytes${expectedSize}).`, { name: "AbiEncodingBytesSizeMismatchError" });
  }
}

class AbiEncodingLengthMismatchError2 extends BaseError4 {
  constructor({ expectedLength, givenLength }) {
    super([
      "ABI encoding params/values length mismatch.",
      `Expected length (params): ${expectedLength}`,
...
class BytesSizeMismatchError3 extends BaseError4 {
  constructor({ expectedSize, givenSize }) {
    super(`Expected bytes${expectedSize}, got bytes${givenSize}.`, {
      name: "BytesSizeMismatchError"
    });
...
class InvalidAbiEncodingTypeError2 extends BaseError4 {
...
class InvalidArrayError3 extends BaseError4 {
  constructor(value) {
    super([`Value "${value}" is not a valid array.`].join(`
`), {
      name: "InvalidArrayError"
...
function prepareParam2({ param, value }) {
  const arrayComponents = getArrayComponents3(param.type);
  if (arrayComponents) {
    const [length, type] = arrayComponents;
    return encodeArray3(value, { length, param: { ...param, type } });
  }
  if (param.type === "tuple") {
    return encodeTuple3(value, {
      param
    });
  }
  if (param.type === "address") {
    return encodeAddress3(value);
  }
  if (param.type === "bool") {
  ...
  if (param.type.startsWith("uint") || param.type.startsWith("int")) {
    const signed = param.type.startsWith("int");
    const [, , size22 = "256"] = integerRegex4.exec(param.type) ?? [];
    return encodeNumber3(value, {
      signed,
      size: Number(size22)
    });
  }
  if (param.type.startsWith("bytes")) {
    return encodeBytes3(value, { param });
  }
  if (param.type === "string") {
    return encodeString3(value);
  }
...
function encodeArray3(value, { length, param }) {
  const dynamic = length === null;
  if (!Array.isArray(value))
...
function encodeBytes3(value, { param }) {
  const [, paramSize] = param.type.split("bytes");
  ...
  if (!paramSize) {
    ...
    return {
      dynamic: true,
      encoded: concat3([padHex2(numberToHex2(bytesSize, { size: 32 })), value_])
    };
  }
  if (bytesSize !== Number.parseInt(paramSize, 10))
...
function encodeNumber3(value, { signed, size: size22 = 256 }) {
  if (typeof size22 === "number") {
    const max = 2n ** (BigInt(size22) - (signed ? 1n : 0n)) - 1n;
    const min = signed ? -max - 1n : 0n;
    if (value > max || value < min)
      throw new IntegerOutOfRangeError3({
        max: max.toString(),
...
function encodeString3(value) {
  ...
  const parts = [];
  for (let i = 0;i < partsLength; i++) {
    parts.push(padHex2(slice4(hexValue, i * 32, (i + 1) * 32), {
      dir: "right"
    }));
  }
  return {
    dynamic: true,
    encoded: concat3([
      padHex2(numberToHex2(size5(hexValue), { size: 32 })),
      ...parts
    ])
  };
}
function encodeTuple3(value, { param }) {
  let dynamic = false;
  const preparedParams = [];
  for (let i = 0;i < param.components.length; i++) {
    const param_ = param.components[i];
  ...
  return {
    dynamic,
    encoded: dynamic ? encodeParams2(preparedParams) : concat3(preparedParams.map(({ encoded }) => encoded))
  };
}
...
class InvalidDomainError2 extends BaseError4 {
  constructor({ domain }) {
...
class InvalidPrimaryTypeError2 extends BaseError4 {
  constructor({ primaryType, types }) {
    super(`Invalid primary type \`${primaryType}\` must be one of \`${JSON.stringify(Object.keys(types))}\`.`, {
      docsPath: "/api/glossary/Errors#typeddatainvalidprimarytypeerror",
      metaMessages: ["Check that the primary type is a key in `types`."]
...
class InvalidStructTypeError2 extends BaseError4 {
  constructor({ type }) {
    super(`Struct type "${type}" is invalid.`, {
      metaMessages: ["Struct type must not be a Solidity type."],
      name: "InvalidStructTypeError"
...
function validateTypedData2(parameters) {
  ...
  const validateData = (struct, data) => {
    for (const param of struct) {
      const { name, type } = param;
      const value = data[name];
      const integerMatch = type.match(integerRegex4);
      if (integerMatch && (typeof value === "number" || typeof value === "bigint")) {
        const [_type, base, size_] = integerMatch;
        numberToHex2(value, {
          signed: base === "int",
      ...
      if (bytesMatch) {
        const [_type, size_] = bytesMatch;
        if (size_ && size5(value) !== Number.parseInt(size_, 10))
          throw new BytesSizeMismatchError3({
            expectedSize: Number.parseInt(size_, 10),
            givenSize: size5(value)
          });
      }
      const struct2 = types[type];
      if (struct2) {
...
function hashTypedData2(parameters) {
  ...
  return keccak2563(concat3(parts));
}
...
function encodeData3({ data, primaryType, types }) {
  const encodedTypes = [{ type: "bytes32" }];
...
function hashType2({ primaryType, types }) {
  const encodedHashType = toHex3(encodeType2({ primaryType, types }));
...
function findTypeDependencies2({ primaryType: primaryType_, types }, results = new Set) {
  const match = primaryType_.match(/^\w*/u);
  const primaryType = match?.[0];
  if (results.has(primaryType) || types[primaryType] === undefined) {
    return results;
...
function encodeField2({ types, name, type, value }) {
  if (types[type] !== undefined) {
    return [
      { type: "bytes32" },
      keccak2563(encodeData3({ data: value, primaryType: type, types }))
    ];
  }
  if (type === "bytes")
    return [{ type: "bytes32" }, keccak2563(value)];
  if (type === "string")
    return [{ type: "bytes32" }, keccak2563(toHex3(value))];
  if (type.lastIndexOf("]") === type.length - 1) {
    const parsedType = type.slice(0, type.lastIndexOf("["));
    ...
    return [
      { type: "bytes32" },
      keccak2563(encodeAbiParameters2(typeValuePairs.map(([t]) => t), typeValuePairs.map(([, v]) => v)))
    ];
  }
  return [{ type }, value];
}
...
function privateKeyToAccount(privateKey, options = {}) {
  ...
  const publicKey = toHex3(secp256k12.getPublicKey(privateKey.slice(2), false));
  const address = publicKeyToAddress2(publicKey);
...
var CELO_SEPOLIA_RPC = "https://forno.celo-sepolia.celo-testnet.org";
...
var agentAccount = loadAgentAccount();
var MATCHER_CONFIG = {
  amountTolerancePct: 10,
  minGroupSize: 3,
  maxGroupSize: 20,
  intentLookbackSec: 7 * 24 * 60 * 60
};
var CYCLE_DURATIONS = {
  weekly: 7 * 24 * 60 * 60,
...
var DRY_RUN = process.env.DRY_RUN === "true" || process.env.DRY_RUN === "1";
...
var walletClient = agentAccount ? createWalletClient({
  account: agentAccount,
  chain: celoSepolia,
  transport: http(CELO_SEPOLIA_RPC)
}) : null;
var intentRegistryAbi = [
...
function getSaveCircle(address) {
  return getContract({
    address,
    abi: saveCircleAbi,
    client: publicClient
...
var IntentType;
((IntentType2) => {
  IntentType2[IntentType2["JOIN_CIRCLE"] = 0] = "JOIN_CIRCLE";
  IntentType2[IntentType2["LEAVE_CIRCLE"] = 1] = "LEAVE_CIRCLE";
  IntentType2[IntentType2["ADJUST_CONTRIBUTION"] = 2] = "ADJUST_CONTRIBUTION";
})(IntentType ||= {});
var CircleState;
((CircleState2) => {
  CircleState2[CircleState2["FORMING"] = 0] = "FORMING";
  CircleState2[CircleState2["ACTIVE"] = 1] = "ACTIVE";
  CircleState2[CircleState2["PAUSED"] = 2] = "PAUSED";
  CircleState2[CircleState2["COMPLETE"] = 3] = "COMPLETE";
  CircleState2[CircleState2["DISSOLVED"] = 4] = "DISSOLVED";
})(CircleState ||= {});
export {
  walletClient,
  saveCircleAbi,
