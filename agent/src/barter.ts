...
export interface BarterIntent {
  id: string;
  agentAddress: string;
  offering: string; // e.g., "web design (40 hours)"
  seeking: string; // e.g., "childcare (10 hrs/week)"
  description: string; // full description
  createdAt: number; // unix timestamp
  expiresAt?: number; // optional expiry
  status: "OPEN" | "MATCHED" | "SETTLED";
  matchedWith?: string; // matched intent ID
}

/**
 * In-memory barter intent storage (mock)
 */
const barterIntents = new Map<string, BarterIntent>();
let intentCounter = 0;
 ...
 * @returns Intent ID
 */
export async function submitBarterIntent(
  agentAddress: string,
  offering: string,
  seeking: string,
  expiresAt?: number
  ...
  addActivity(
    "INTENT_PARSED",
    `Barter intent submitted: ${intentId}`,
    `${agentAddress.slice(0, 6)}... offering "${offering}", seeking "${seeking}"`,
    0.9
  );
 ...
 */
export async function matchBarterIntents(): Promise<
  Array<{
    matchId: string;
...
> {
  ...
  if (matches.length > 0) {
    addActivity(
      "CIRCLE_MATCH",
      `Found ${matches.length} barter match(es)`,
      `Top match: ${matches[0].matchId} (${matches[0].compatibility}% compatibility)`,
 ...
 */
export async function settleBarterIntent(intentAId: string, intentBId: string): Promise<void> {
  ...
  const settledIntentA: BarterIntent = {
    ...intentA,
    status: "SETTLED",
    matchedWith: intentBId,
  };
  const settledIntentB: BarterIntent = {
    ...intentB,
    status: "SETTLED",
  ...
  addActivity(
    "EXECUTE",
    `Barter settlement: ${intentAId} ↔ ${intentBId}`,
    `${intentA.agentAddress.slice(0, 6)}... and ${intentB.agentAddress.slice(0, 6)}... completed barter exchange`,
    1.0
  );
}
 ...
 */
export function getOpenBarterIntents(): BarterIntent[] {
 ...
 */
export function getBarterIntent(intentId: string): BarterIntent | undefined {
 ...
 */
export function getAgentBarterIntents(agentAddress: string): BarterIntent[] {
  return Array.from(barterIntents.values()).filter(
    (i) => i.agentAddress.toLowerCase() === agentAddress.toLowerCase()
  );
}
...
function keywordSimilarity(text1: string, text2: string): number {
  ...
  tokens1.forEach((t) => {
    if (stopWords.has(t)) tokens1.delete(t);
  });
  tokens2.forEach((t) => {
    if (stopWords.has(t)) tokens2.delete(t);
  });
 ...
 */
export function getAllBarterIntents(): BarterIntent[] {
 ...
 */
export function clearBarterIntents(): void {
  barterIntents.clear();
  intentCounter = 0;
}
