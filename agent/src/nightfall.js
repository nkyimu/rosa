 ...
 */
export async function deriveKeys(mnemonic, childPath = "m/44'/60'/0'/0/0") {
    ...
    if (!res.ok)
        throw new Error(`deriveKey failed: ${res.status} ${await res.text()}`);
 ...
 */
export async function deposit(ercAddress, amountWei) {
 ...
 */
export async function transfer(ercAddress, amountWei, recipientZkpPublicKey) {
    const requestId = crypto.randomUUID();
    const valueHex = amountWei.toString(16).padStart(64, '0');
 ...
 */
export async function withdraw(ercAddress, amountWei, recipientAddress) {
    const requestId = crypto.randomUUID();
    const valueHex = amountWei.toString(16).padStart(64, '0');
 ...
 */
export async function processContribution(memberAddress, circleAddress, amount, salt) {
    const cUSD = CONTRACT_ADDRESSES.cUSD;
 ...
 */
export async function processPayout(recipientAddress, amount) {
    const cUSD = CONTRACT_ADDRESSES.cUSD;
