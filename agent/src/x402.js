 ...
 * ```
 */
export async function getPaymentRequired(agentPaymentAddress) {
    const contractAddress = agentPaymentAddress || CONTRACT_ADDRESSES.agentPayment;
 ...
 * ```
 */
export async function verifyPayment(intentId, agentPaymentAddress) {
    try {
        const contractAddress = agentPaymentAddress || CONTRACT_ADDRESSES.agentPayment;
        if (!contractAddress || contractAddress === "0x0000000000000000000000000000000000000000") {
            return {
 ...
 * ```
 */
export function x402Middleware(agentPaymentAddress) {
    return async (req, res, next) => {
        try {
            ...
            // Verify the payment
            const verification = await verifyPayment(parseInt(intentId, 10), agentPaymentAddress);
 ...
 */
export async function buildPaymentRequiredResponse(agentPaymentAddress) {
 ...
 */
export async function paymentRequired(intentId, agentPaymentAddress) {
 ...
 */
export async function getCurrentFee(agentPaymentAddress) {
    const contractAddress = agentPaymentAddress || CONTRACT_ADDRESSES.agentPayment;
    if (!contractAddress || contractAddress === "0x0000000000000000000000000000000000000000") {
        throw new Error("AgentPayment contract not deployed");
    }
