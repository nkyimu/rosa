...
```json
{
  ...
  "description": "ROSA is an autonomous agent that creates and manages private savings circles (ROSCAs) on Celo. It matches compatible savers via intent-based grouping, deploys SaveCircle smart contracts, enforces contributions with penalties, rotates payouts, and hides contribution amounts via ZK commit-reveal. Built on Celo for mobile-first access, ROSA eliminates the need for a human coordinator — the agent handles everything from circle formation to payout distribution.\n\nKey features:\n- Intent-based matching: Submit what you want to save, ROSA finds compatible members\n- Autonomous circle management: Deploy, enforce, rotate — no human coordinator\n- Privacy-preserving contributions: ZK commit-reveal hides individual amounts\n- On-chain trust scoring: Three-dimensional (reliability, credit, community)\n- x402 payment integration: Agent services accessible via standard HTTP payments\n- ERC-8004 agent identity: Verifiable on-chain registration\n- MiniPay compatible: Mobile-first for Celo's 2.5M+ wallet users",
  "problemStatement": "2 billion people worldwide participate in informal savings circles (ROSCAs, susus, tandas, chit funds). These circles depend entirely on a human coordinator who carries all the risk — chasing late payments, managing trust disputes, handling rotation schedules. When the coordinator fails or disappears, the entire circle collapses and members lose their savings. There's no way to build portable reputation across circles, no privacy for contribution amounts, and no credit access for reliable savers. ROSA solves this by replacing the coordinator with an autonomous agent that enforces rules via smart contracts, keeps contributions private, and builds on-chain trust scores that persist across circles.",
